...
vector fireCenterPosiblePosition(s_room* room) {
  ...
  vector commonPoint;
  vector_init(&possiblePosition);
  vector_init(&commonPoint);

  int pointPlaced = 0;

  for(int i=room->robot.pos.x-5; i<=room->robot.pos.x+5; i++)
    for(int j=room->robot.pos.y-5; j<=room->robot.pos.y+5; j++) {
      if(i<0 || j<0 || i>room->sizeX || j>room->sizeY)
        continue;

      if(room->nodes[j][i].robotVision != TILE_FIRE_LVL1 && room->nodes[j][i].robotVision != TILE_FIRE_LVL2)
        continue;

      for(int a=-2+i; a<=2+i; a++)
        for(int b=-2+j; b<=2+j; b++) {
          if(a<0 || b<0 || a>=room->sizeX || b>=room->sizeY)
            continue;

          float dist = sqrt(pow(a-i, 2) + pow(b-j, 2));
          if((dist > 0 && dist <= 1 && room->nodes[j][i].robotVision == TILE_FIRE_LVL2)
            || (dist > 0 && dist <= 2 && room->nodes[j][i].robotVision == TILE_FIRE_LVL1)) {
                ...
                if(!pointPlaced) {
                  ...
                  for(int k=0; k<vector_total(&possiblePosition); k++)
                    if(temp == vector_get(&possiblePosition, k))
                      vector_add(&commonPoint, temp);
                }
            }
      }

      if(pointPlaced) {
        vector_free(&possiblePosition);
        vector_init(&possiblePosition);

        for(int k=0; k<vector_total(&commonPoint); k++)
          vector_add(&possiblePosition, vector_get(&commonPoint, k));
